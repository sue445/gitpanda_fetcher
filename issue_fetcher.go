...
func (f *issueFetcher) fetchPath(path string, client *gitlab.Client, isDebugLogging bool) (*Page, error) {
	...
	eg.Go(func() error {
		...
		issue, err = WithDebugLogging("issueFetcher(GetIssue)", isDebugLogging, func() (*gitlab.Issue, error) {
		...
		if matched2 != nil {
			note, err := WithDebugLogging("noteFetcher(GetIssueNote)", isDebugLogging, func() (*gitlab.Note, error) {
